param(
    ...
    [string]$ProjectName = "career-agent"
)
...
if ($Environment -eq "prod") { # Use prod.tfvars for extra settings in production environment
...
"NEXT_PUBLIC_API_URL=$ApiUrl" | Out-File .env.production -Encoding utf8 # Writes "NEXT_PUBLIC_API_URL=<api-url>" to .env.production file
...
# 4. Final summary
$CfUrl = terraform -chdir=terraform output -raw cloudfront_url # chdir is used to run terraform command from the terraform directory without manually changing the location
